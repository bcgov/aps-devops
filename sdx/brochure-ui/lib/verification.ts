// Server-side verification logic for SDX activity log entries.
//
// Two distinct checks are exposed, each performed independently on the
// request and response sides:
//   * X-Edge-Token  — a JWT whose payload carries a `jwks_uri` claim
//     (OIDC-style). The token is verified against that JWKS.
//   * X-Entity-Sig  — a detached signature over the X-Edge-Token's third
...
//     (`<env>.<memberclass>.<memberid>.<subsystem>`) — taken from
//     X-Client-ID (request side) or the Kong route's `service:` tag
//     (response side) — and mapped to the keyset name
...
export interface SidedResult {
  request: VerificationResult;
  response: VerificationResult;
}

export interface ActivityVerification {
  edgeToken: SidedResult;
  entitySig: SidedResult;
}
...
// JWKS endpoints are cached for the lifetime of the server process: once a
// JWKS URL has been resolved successfully, every subsequent verification
// reuses the same in-memory copy. Concurrent first-time requests for the
// same URL share a single in-flight Promise so we never fire duplicate
// fetches against the keyset endpoint.
const jwksCache = new Map<string, Jwks>();
const jwksInflight = new Map<string, Promise<Jwks>>();

// Drop any cached JWKS responses so the next verification re-fetches them.
// Called when a request asks for `?refresh=1`, since deployed keysets and
// configured JWKS endpoints can change between page loads.
...
function parseJwt(token: string): JwtParts {
  ...
  if (parts.length !== 3) throw new Error("not a JWT (expected 3 segments)");
  const [h, p, s] = parts;
  const header = JSON.parse(b64urlToText(h));
  const payload = JSON.parse(b64urlToText(p));
  const signature = b64urlToBytes(s);
  const signingInput = new TextEncoder().encode(`${h}.${p}`);
  return { header, payload, signature, signingInput, segments: [h, p, s] };
}
...
function pickKey(keys: Jwk[], kid: string | undefined): Jwk[] {
  // Prefer a kid match; fall back to all keys (some issuers omit kid).
  if (kid) {
...
async function verifyJwtAgainstJwks(
  ...
  if (!alg) return { ok: false, alg: "?", reason: "missing alg in header" };

  let params: AlgParams;
  try {
    params = algToParams(alg);
  } catch (e) {
    return { ok: false, alg, reason: (e as Error).message };
  }

  const candidates = pickKey(jwks.keys, parsed.header.kid);
  if (candidates.length === 0) {
    return { ok: false, alg, kid: parsed.header.kid, reason: "no JWKS keys" };
  }
  ...
  for (const jwk of candidates) {
    try {
      ...
      if (ok) return { ok: true, alg, kid: parsed.header.kid, jwk };
    } catch {
      // try next key
    }
  }
  return { ok: false, alg, kid: parsed.header.kid, reason: "no key verified" };
}
...
export async function verifyEdgeToken(
  ...
  try {
    ...
    return { status: "invalid", source, message: (e as Error).message };
  }
  const jwksUrl = parsed.payload?.jwks_uri;
  if (typeof jwksUrl !== "string" || !jwksUrl) {
    return {
      status: "invalid",
      source,
      message: "token has no jwks_uri claim",
    };
  }
  let jwks: Jwks;
  try {
    ...
    return {
      status: "error",
      source,
      message: `JWKS fetch failed: ${(e as Error).message}`,
      details: { jwks_url: jwksUrl },
    };
  }
  const result = await verifyJwtAgainstJwks(token, jwks);
  const details: Record<string, string> = {
    jwks_uri: jwksUrl,
    alg: result.alg,
  };
  if (result.kid) details.kid = result.kid;
  if (typeof parsed.payload.iss === "string") details.iss = parsed.payload.iss;
  if (typeof parsed.payload.sub === "string") details.sub = parsed.payload.sub;
  if (result.ok) await applyCertChainCheck(details, result.jwk);
  return result.ok
    ? {
        status: "valid",
        source,
        message: "edge token signature verified",
        details,
      }
    : {
        status: "invalid",
...
// Map a service (response-side) dotted SDX id, which carries the environment as
// its leading segment (`<env>.<memberclass>.<memberid>.<subsystem>`,
// e.g. "LAB.MIN.CITZ.SDG-FE"), to its org keyset name
// (`sdx.org.<memberclass_lower>.<memberid_lower>.<env_lower>`,
...
// The environment segment (leading part, lowercased) of a service dotted id
// (`<env>.<class>.<id>.<subsystem>`). The request-side X-Client-ID does not
// carry an environment, so both sides take it from the service id.
export function envFromServiceDotted(dotted: string): string | undefined {
  const parts = dotted.split(".");
  if (parts.length < 4) return undefined;
  return parts[0]?.toLowerCase() || undefined;
}

// Map a request-side X-Client-ID (`<memberclass>.<memberid>.<subsystem>`, with
// NO environment segment, e.g. "USR.AC.MY-NEW-SUBSYSTEM") plus the event
// environment (taken from the service id) to the org keyset name
// `sdx.org.<memberclass_lower>.<memberid_lower>.<env_lower>`
// (e.g. "sdx.org.usr.ac.dev").
export function clientIdToOrgKeyset(
  clientId: string,
  env: string,
): string | undefined {
  const parts = clientId.split(".");
  const memberclass = parts[0]?.toLowerCase();
  const memberid = parts[1]?.toLowerCase();
  if (!memberclass || !memberid || !env) return undefined;
  return `sdx.org.${memberclass}.${memberid}.${env.toLowerCase()}`;
}

// Find the first tag matching `prefix:` and return the value after the colon.
// deno-lint-ignore no-explicit-any
function findTagValue(tags: any, prefix: string): string | undefined {
  if (!Array.isArray(tags)) return undefined;
  const p = prefix + ":";
  for (const t of tags) {
    if (typeof t === "string" && t.startsWith(p)) return t.slice(p.length);
  }
  return undefined;
}
...
export async function verifyEntitySig(
  ...
  edgeToken: string,
  entity: { dotted: string; keysetId: string },
  source: HeaderSource,
): Promise<VerificationResult> {
  ...
  try {
    ...
    return {
      ...
      details: {
        entity: entity.dotted,
        keyset: entity.keysetId,
        jwks_uri: url,
      },
    };
  }
...
interface ExtractedHeaders {
  edgeToken: { request?: string; response?: string };
  entitySig: { request?: SigInfo; response?: SigInfo };
}

// deno-lint-ignore no-explicit-any
function extractHeaders(value: any): ExtractedHeaders {
  const reqHeaders = value?.request?.headers ?? {};
  const resHeaders = value?.response?.headers ?? {};
  const routeHeaders = value?.route?.headers ?? {};
  const routeTags = value?.route?.tags;
  const serviceTags = value?.service?.tags;

  const edgeReq =
    findHeader(reqHeaders, "X-Edge-Token") ??
    findHeader(routeHeaders, "X-Edge-Token");
  const edgeRes = findHeader(resHeaders, "X-Edge-Token");

  const sigReq =
    findHeader(reqHeaders, "X-Entity-Sig") ??
    findHeader(routeHeaders, "X-Entity-Sig");
  const sigRes = findHeader(resHeaders, "X-Entity-Sig");

  // The response-side service id carries the environment as its leading segment
  // (`<env>.<class>.<id>.<subsystem>`); the request-side X-Client-ID does not.
  // Derive the event environment from the service id and share it with both
  // sides.
  const serviceDotted =
    findTagValue(routeTags, "service") ??
    findTagValue(serviceTags, "service");
  const env = serviceDotted ? envFromServiceDotted(serviceDotted) : undefined;

  const entitySig: ExtractedHeaders["entitySig"] = {};
  if (sigReq) {
    // Request side: dotted entity is the X-Client-ID
    // (`<class>.<id>.<subsystem>`, no environment), with a fallback to the
    // route's `client:` tag. The environment comes from the service id.
    const dotted =
      findHeader(reqHeaders, "X-Client-ID") ??
      findHeader(reqHeaders, "X-Client-Id") ??
      findTagValue(routeTags, "client") ??
      findTagValue(serviceTags, "client");
    entitySig.request = {
      value: sigReq,
      dotted,
      keysetId: dotted && env ? clientIdToOrgKeyset(dotted, env) : undefined,
    };
  }
  if (sigRes) {
    // Response side: dotted entity is the service id
    // (`<env>.<class>.<id>.<subsystem>`), exposed via the `service:` Kong tag.
    entitySig.response = {
      value: sigRes,
      dotted: serviceDotted,
      keysetId: serviceDotted ? dottedEntityToKeyset(serviceDotted) : undefined,
    };
  }

  return {
    edgeToken: { request: edgeReq, response: edgeRes },
    entitySig,
  };
}

async function verifySidedEntitySig(
  source: HeaderSource,
  sig: SigInfo | undefined,
  edgeTokenForBinding: string | undefined,
): Promise<VerificationResult> {
  if (!sig) {
    return {
      status: "missing",
      source,
      message: `no X-Entity-Sig on ${source}`,
    };
  }
  if (!sig.dotted) {
    return {
      ...
      message:
        source === "request"
          ? "no X-Client-ID (or client tag) to derive keyset"
          : "no service tag to derive keyset",
    };
  }
  if (!sig.keysetId) {
    return {
      status: "invalid",
      source,
      message: `dotted id '${sig.dotted}' is not in <env>.<class>.<id>.<sub> form`,
      details: { entity: sig.dotted },
    };
  }
  if (!edgeTokenForBinding) {
    return {
      status: "invalid",
      source,
      message: "no X-Edge-Token to bind entity sig to",
    };
  }
  return await verifyEntitySig(
    sig.value,
    edgeTokenForBinding,
    { dotted: sig.dotted, keysetId: sig.keysetId },
    source,
  );
}

export async function verifyActivityEvent(
  // deno-lint-ignore no-explicit-any
  value: any,
): Promise<ActivityVerification | undefined> {
  const { edgeToken, entitySig } = extractHeaders(value);
  const anyHeader =
    edgeToken.request ||
    edgeToken.response ||
    entitySig.request ||
    entitySig.response;
  if (!anyHeader) return undefined;

  const edgeReqResult: VerificationResult = edgeToken.request
    ? await verifyEdgeToken(edgeToken.request, "request")
    : {
        status: "missing",
        source: "request",
        message: "no X-Edge-Token on request",
      };
  const edgeResResult: VerificationResult = edgeToken.response
    ? await verifyEdgeToken(edgeToken.response, "response")
    : {
        status: "missing",
        source: "response",
        message: "no X-Edge-Token on response",
      };

  const sigReqResult = await verifySidedEntitySig(
    "request",
    entitySig.request,
    edgeToken.request ?? edgeToken.response,
  );
  const sigResResult = await verifySidedEntitySig(
    "response",
    entitySig.response,
    edgeToken.response ?? edgeToken.request,
  );

  return {
    edgeToken: { request: edgeReqResult, response: edgeResResult },
    entitySig: { request: sigReqResult, response: sigResResult },
  };
}
